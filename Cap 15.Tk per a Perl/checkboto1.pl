...
#Etiqueta:
my $etiqueta = $marc->new_ttk__label(-text => "Text de l'etiqueta",
                                     -style => "Estil1.TLabel");
$etiqueta->g_grid(-column => 0, -row => 0, -sticky => "w");
#CheckBotó:
my $resultsVar;
my $checkboto = $marc->new_ttk__checkbutton(-text => "Text del checkbutton",
                      -variable => \$resultsVar, -onvalue => "Text onvalue",
                      -offvalue => "Text offvalue", -command => \&canviaestil);
$checkboto->g_grid(-column => 0, -row => 1, -sticky => "w");
#Acció:
sub canviaestil {
